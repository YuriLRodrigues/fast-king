...
const OrderList = ({ orders }: OrderListProps) => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {orders.map((order) => (
        <Card key={order.id}>
          <CardContent className="justify space-y-4 p-5">
            ...
            <div className="space-y-2">
              {order.orderProducts.map((orderProduct) => (
                <div key={orderProduct.id} className="flex items-center gap-2">
                  <div className="flex h-5 w-5 items-center justify-center rounded-full bg-gray-400 text-xs font-normal text-white">
